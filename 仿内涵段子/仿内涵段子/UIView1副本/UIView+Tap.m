...
static const char *ActionHandlerTapGestureKey1;
...
- (void)setTapActionWithBlock:(void (^)(void))block {
    ...
    objc_setAssociatedObject(self, &ActionHandlerTapGestureKey1, block, OBJC_ASSOCIATION_COPY);
}

- (void)handleActionForTapGesture:(UITapGestureRecognizer *)gesture {
    if (gesture.state == UIGestureRecognizerStateRecognized)  {
        void(^action)(void) = objc_getAssociatedObject(self, &ActionHandlerTapGestureKey1);
        if (action)  {
            action();
        }
    }
