...
- (id)initWithFrame:(CGRect)frame
{
    if ((self = [super initWithFrame:frame])) {
        ...
        // 图片
        _imageView = [[SCXBaseImageView alloc] init];
...
- (void)longPress:(UILongPressGestureRecognizer *)longPressGest {
    
    SCXCustomActionSheetView *actionSheet = [SCXCustomActionSheetView customActionSheetWithActionTitle:@"保存图片" andCancelTitle:@"取消" andSutitles:@"保存图片到相册", nil];
    [actionSheet show];
    __block typeof(self)weakSelf=self;
    [actionSheet setActionSheetClickHandleBlock:^(SCXCustomActionSheetView *actionSheet, NSInteger currentIndex, NSString *title) {
        if (currentIndex==0) {
            if ([weakSelf.photoViewDelegate respondsToSelector:@selector(photoViewDidLongPress:)]) {
                [weakSelf.photoViewDelegate photoViewDidLongPress:weakSelf];
            }
        }
    }];
...
- (void)showImage
{
    if (_photo.firstShow) { // 首次显示
        ...
        if (![_photo.url.absoluteString hasSuffix:@"gif"]) {
            ...
            [_imageView setImageWithURL:_photo.url placeHolder:_photo.placeholder finishHandle:^(bool finished, UIImage *image) {
...
- (void)photoStartLoad
{
    if (_photo.image) {
        ...
        //        }];
        [_imageView setImageWithURL:_photo.url placeHolder:_photo.srcImageView.image finishHandle:^(bool finish, UIImage *image) {
            dispatch_async(dispatch_get_main_queue(), ^{
